...
if(isset($_GET['send']) ) {
	if(!isset($_POST['email']) || empty($_POST['email'])) {
		...
		if($user === false) {
			...
			$betreff = "Neues Passwort für deinen Account auf Audioboox"; //Ersetzt hier den Domain-Namen
			$from = "From: Audioboox <audioboox@server4.bplaced.net>"; //Ersetzt hier euren Name und E-Mail-Adresse
...
für deinen Account auf Audioboox wurde nach einem neuen Passwort gefragt. Um ein neues Passwort zu vergeben, rufe innerhalb der nächsten 24 Stunden die folgende Website auf:
'.$url_passwortcode.'
